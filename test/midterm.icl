...
// Sebastián Ignacio Arriagada Silva
// HSO14Z
//Functional Programming & mid-term-retake
//2021.September.14 
//This solution was submitted and prepared by <Sebastián Ignacio Arriagada Silva, HSO14Z> 
...
= False
where 
      sumatoria = cubeSume (fromIntToList num)

cubeSume :: [Int] -> Int
cubeSume lst 
|lst == [] = 0
= (hd lst) ^ 3 + cubeSume (tl lst )
...
counter2 :: [Int] [Int] -> [Int]
counter2 lst1 lst2
| lst2 == [] = []
=  [counter lst1 (hd lst2) 0 ]  ++ counter2 lst1 (tl lst2)

counter :: [Int] Int Int -> Int
counter lst num count 
| isMember num lst =  0 + counter (removeMember num lst) num (count + 1)
= count
...
|lst == [] = []
| length lst == 1 = lst
| length lst == 2 = gap2Aux (lst!!0) (lst!!1)
= removeDup ( gap2Aux (lst!!0) (lst!!1) ++ ( gap2 (drop 1 lst )))

gap2Aux :: Int Int -> [Int]
gap2Aux num1 num2
| num1 <= num2 = [num1] ++ gap2Aux (num1 + 2) num2
= []
...
getRidPal lst 
| lst == [] = []
= [filter getRidPalAux (hd lst )] ++ getRidPal (tl lst )

getRidPalAux :: Int -> Bool
getRidPalAux num
| num < 10 = True
| (fromIntToList num) == reverse (fromIntToList num) = False
= True
...
removeNotPrime lst = filter ( not o isPrime) lst
...
collatzAux num count
| num == 2 = count 
| isEven num = 0 + collatzAux (num/2) (count + 1)
= 0 + collatzAux (3 * num + 1) (count + 1)

//Start = collatz 10 // 4
//Start = collatz 20000000 // 144
//Start = collatz 5 // 3
//Start = collatz 0 // "The number must be greater than 1" -1
...
isGood lst 
| lst == [] = 0
= isGoodAux (hd lst) + isGood (tl lst)

isGoodAux :: [Int] -> Int
isGoodAux lst 
| length lst == 0 = 1
| length lst == 1 && isOdd (lst!!0) = 1
| length lst == 2 && isOdd (lst!!0) && isEven (lst!!1) = 1
| length lst == 3 && isOdd (lst!!0) && isEven (lst!!1) && isOdd (lst!!2)= 1 
| length lst > 3 && isOdd (lst!!0) && isEven (lst!!1) = isGoodAux (drop 2 lst)
= 0
...
symSumGreater10 lst 
| lst == [] = []
| (sum (hd lst) ) > 10 = [ (hd lst) ++ reverse (hd lst) ] ++ symSumGreater10 (tl lst )
= symSumGreater10 (tl lst )
...
elementInInterval lst 
| lst == [] = []
= [tupleElements ( hd lst )] ++ elementInInterval ( tl lst) 

tupleElements :: (Int, Int, [Int] ) -> [Int]
tupleElements tpl = take ( (snd3 tpl) - (fst3 tpl) + 1 ) ( drop (fst3 tpl) ( thd3 tpl) )
